...
void testsek()
{
    ...
    // pointerdestroy and arraydestroy
    {
        ...
        int *pi = new int(123);
        pointerdestroy(pi);
        cout << "pointerdestroy set pointer to " << (pi == nullptr ? "nullptr" : "non-null");
        assert(pi == nullptr);
        cout << " [OK]\n";
        ...
        arr[2] = 3.3;
        arraydestroy(arr);
        cout << "arraydestroy set pointer to " << (arr == nullptr ? "nullptr" : "non-null");
        assert(arr == nullptr);
        cout << " [OK]\n";
    }
    ...
    // arraycreate with corner cases
    {
        ...
        arraycreate(px, 0);
        cout << "arraycreate with size 0 yields " << (px == nullptr ? "nullptr pointer" : "allocated pointer");
        assert(px == nullptr);
        cout << " [OK]\n";
    }
    ...
    // tupleforcycle
    {
        ...
        assert(elems.size() == 3 && elems[0] == "1" && elems[1] == "A" && elems[2] == "2.5");
        cout << " [OK]\n";
    }
    ...
    // file utilities: fileifexists, fileopenoutput, fileopeninput
    {
        ...
        cout << "fileifexists on \"" << filename << "\" (not present) -> " << (exists ? "true" : "false");
        assert(!exists);
        cout << " [OK]\n";
    ...
    // commandinvoke
    {
        ...
        // Use a simple shell command to output "OK" to a file (works on Windows/Linux)
    ...
    // Constructors and size management
    {
        ...
        seque<int> s5val(5, 7);
        cout << "seque<int>(5,7): size = " << s5val.size << ", first element = " << s5val[0];
    ...
    // Copy and move constructors, assignment operators
    {
        ...
        seque<int> moveCtor(move(orig));
        cout << "move constructed seque: size=" << moveCtor.size;
        ...
        assignDst = move(assignSrc);
        cout << "move assignment: dst size=" << assignDst.size;
        assert(assignDst.size == 3 && assignDst[0] == 5);
        assert(assignSrc.size == 0);
        cout << " [OK]\n";
    }
    ...
    // operator= with a value and initializer_list
    {
        ...
        s = {7, 8};
        cout << "after s = {7,8}: size=" << s.size << ", elements: " << s[0] << ", " << s[1];
    ...
    // append: appending values and containers, including corner cases with empty seque
    {
    ...
    // Testing algorithms on empty seque
    {
        ...
        // isfullysatisfied on empty seque should typically return true (vacuous truth)
        seque<int> empty;
