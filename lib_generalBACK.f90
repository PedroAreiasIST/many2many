...
!*** FILES
!---------
  INTEGER, DIMENSION(1000)::types
  PRIVATE::types, fileint, filedouble, filelogical, filestring, fileintvec, filedoublevec, filestringvec, fileintvec2, fileintvec3, fileintvec4, filedoublevec2, filedoublevec3, filedoublevec4
  INTERFACE iotransfer
    MODULE PROCEDURE fileint, filedouble, filelogical, filestring, fileintvec, filedoublevec, filestringvec, fileintvec2, fileintvec3, fileintvec4, filedoublevec2, filedoublevec3, filedoublevec4
...
!> use Gerschgorin circle theorem for the determination of critical time step
  FUNCTION elementcriticaltimestep(meq, neq, emat, emas, indmaps)
    IMPLICIT REAL(8) (a-h, o-z)
...
!> from a basis, determines an orthogonal and normed basis
!> lowdin method
...
!> from a xi3 in [-1,1] and a number of layers nlayer,
!> return layer number
  INTEGER FUNCTION elementlayernumber(xi3, nlayer)
    IMPLICIT REAL(8) (a-h, o-z)
    REAL(8), PARAMETER::xmi = -1.0d00
    REAL(8), PARAMETER::xma = +1.0d00
    nn = int(nlayer*(xi3-xmi)/(xma-xmi)+1)
    nn = min(max(1, nn), nlayer)
    elementlayernumber = nn
  END FUNCTION elementlayernumber
!> tri or tet strain
...
!> shape functions and derivatives (cartesian) for a 4 node tet
...
!> shape functions and derivatives (cartesian) for a 3 node tri
...
!> brent equation solver (Numerical Recipes)
...
!> iter (out)          number of performed substeps
  SUBROUTINE equationdynrelax(miter, na, a, sola, nq, q, solq, tol, iter)
    IMPLICIT REAL(8) (a-h, o-z)
    REAL(8)::tol
    REAL(8), DIMENSION(na)::a, ra
    REAL(8), DIMENSION(nq)::q, qtil, rq, rqold, dr, rq0
    EXTERNAL::sola, solq
!> solution stage
    CALL sola(na, a)
    CALL solq(nq, q, qtil)
...
!> close a given channel from the types list
  SUBROUTINE filecloseinc(ichannel)
    CLOSE (ichannel)
    types(ichannel) = 0
  END SUBROUTINE filecloseinc
!> count lines up to a string
  ...
  SUBROUTINE filedouble(ichannel, val)
    REAL(8)::val
    IF (types(ichannel) .EQ. 0) THEN
      WRITE (ichannel) val
    ELSE
      READ (ichannel) val
    END IF
  END SUBROUTINE filedouble
  SUBROUTINE filedoublevec2(ichannel, dim1, dim2, array)
    INTEGER::dim1, dim2, dim
    REAL(8), DIMENSION(dim1, dim2)::array
    dim = dim1*dim2
    CALL filedoublevec(ichannel, dim, array)
  END SUBROUTINE filedoublevec2
  SUBROUTINE filedoublevec3(ichannel, dim1, dim2, dim3, array)
    INTEGER::dim1, dim2, dim3, dim
    REAL(8), DIMENSION(dim1, dim2, dim3)::array
    dim = dim1*dim2*dim3
    CALL filedoublevec(ichannel, dim, array)
  END SUBROUTINE filedoublevec3
  SUBROUTINE filedoublevec4(ichannel, dim1, dim2, dim3, dim4, array)
    INTEGER::dim1, dim2, dim3, dim4, dim
    REAL(8), DIMENSION(dim1, dim2, dim3, dim4)::array
    dim = dim1*dim2*dim3*dim4
    CALL filedoublevec(ichannel, dim, array)
  END SUBROUTINE filedoublevec4
  SUBROUTINE filedoublevec(ichannel, dim, array)
    INTEGER::dim
    REAL(8), DIMENSION(*)::array
    IF (types(ichannel) .EQ. 0) THEN
      WRITE (ichannel) array(1:dim)
    ELSE
      READ (ichannel) array(1:dim)
    END IF
  END SUBROUTINE filedoublevec
  LOGICAL FUNCTION fileifexists(str)
    CHARACTER(*)::str
    INQUIRE (file=trim(adjustl(str)), exist=fileifexists)
  END FUNCTION fileifexists
  SUBROUTINE fileint(ichannel, val)
    INTEGER::val
    IF (types(ichannel) .EQ. 0) THEN
      WRITE (ichannel) val
    ELSE
      READ (ichannel) val
    END IF
  END SUBROUTINE fileint
  SUBROUTINE fileintvec2(ichannel, dim1, dim2, array)
    INTEGER::dim1, dim2, dim
    INTEGER, DIMENSION(dim1, dim2)::array
    dim = dim1*dim2
    CALL fileintvec(ichannel, dim, array)
  END SUBROUTINE fileintvec2
  SUBROUTINE fileintvec3(ichannel, dim1, dim2, dim3, array)
    INTEGER::dim1, dim2, dim3, dim
    INTEGER, DIMENSION(dim1, dim2, dim3)::array
    dim = dim1*dim2*dim3
    CALL fileintvec(ichannel, dim, array)
  END SUBROUTINE fileintvec3
  SUBROUTINE fileintvec4(ichannel, dim1, dim2, dim3, dim4, array)
    INTEGER::dim1, dim2, dim3, dim4, dim
    INTEGER, DIMENSION(dim1, dim2, dim3, dim4)::array
    dim = dim1*dim2*dim3*dim4
    CALL fileintvec(ichannel, dim, array)
  END SUBROUTINE fileintvec4
  SUBROUTINE fileintvec(ichannel, dim, array)
    ...
    ELSE
      READ (ichannel) array(1:dim)
    END IF
  END SUBROUTINE fileintvec
  SUBROUTINE filelogical(ichannel, val)
    LOGICAL::val
    INTEGER::temp
    IF (val .EQV. .FALSE.) THEN
      temp = 0
    ELSE
      temp = 1
    END IF
    CALL fileint(ichannel, temp)
  END SUBROUTINE filelogical
...
!> opens file for restart, etc
  INTEGER FUNCTION fileopeninc(name, mode)
    CHARACTER(*)::name
    CHARACTER(len(name))::namet
    CHARACTER::mode
    namet = trim(adjustl(name))!//char(0)
    fileopeninc = fileprovidechannel()
    SELECT CASE (mode)
    CASE ('w', 'W')
      types(fileopeninc) = 0
    CASE ('r', 'R')
      types(fileopeninc) = 1
    END SELECT
    OPEN (fileopeninc, file=namet, status="unknown", ACCESS="STREAM")
  END FUNCTION fileopeninc
!> outputs for CEI ensight and Paraview
!> ASCII only
!> no maintenance
...
!*** check bounds
    IF (allocated(ellist)) DEALLOCATE (ellist)
...
!> io string
  SUBROUTINE filestring(ichannel, val)
    CHARACTER(*)::val
    IF (types(ichannel) .EQ. 0) THEN
      WRITE (ichannel) trim(adjustl(val))
    ELSE
      READ (ichannel) val
    END IF
  END SUBROUTINE filestring
!> io string vector
  SUBROUTINE filestringvec(ichannel, dim, array)
...
!> given xi for a triangle
  REAL(8) FUNCTION geotriangviolation(xi)
    ...
    IF (abs(xi(1)+xi(2)+xi3-1.0d00) .GT. 1.0d-20) geotriangviolation = 1.0d30
...
!> checks if columns are sorted
  LOGICAL FUNCTION manymanychecksorted(na, ia, ja)
    INTEGER na, ia(*), ja(*)
    DO i = 1, na
      DO j = ia(i)+1, ia(i+1)-1
        IF (ja(j-1) .GE. ja(j)) THEN
          mm_checksorted = .FALSE.
          RETURN
        END IF
      END DO
    END DO
    manymanychecksorted = .TRUE.
  END FUNCTION manymanychecksorted
...
!> csr many to many from coordinate representation
  SUBROUTINE manymanyfromcoordinate(num, nnz, ipo, lis, ilin, icol, v1, v2)
    IMPLICIT REAL(8) (a-h, o-z)
    INTEGER, DIMENSION(:), ALLOCATABLE::ipo, lis
    INTEGER, DIMENSION(*)::ilin, icol
    REAL(8), DIMENSION(*), OPTIONAL::v1, v2
    CALL allocsafe(num+1, ipo)
    CALL allocsafe(0, lis)
    DO k = 1, nnz
      ipo(ilin(k)) = ipo(ilin(k))+1
    END DO
    CALL manymanycreaterowpointersandallocate(num, ipo, lis)
    DO k = 1, nnz
      i = ilin(k)
      j = icol(k)
      iad = ipo(i)
      ...
      ipo(i) = iad+1
    END DO
    CALL manymanyrevertrowpointers(num, ipo)
  END SUBROUTINE manymanyfromcoordinate
!> csr many to many conversion to ilin/icol routine
  SUBROUTINE manymanytocoordinate(num, ipo, lis, ilin, icol)
    IMPLICIT REAL(8) (a-h, o-z)
    INTEGER, DIMENSION(*)::ipo, lis
    INTEGER, DIMENSION(:), ALLOCATABLE::ilin, icol
    nnz = ipo(num+1)-ipo(1)
    CALL allocsafe(nnz, ilin)
    CALL allocsafe(nnz, icol)
    DO ir = 1, num
      ...
      END DO
    END DO
  END SUBROUTINE manymanytocoordinate
...
!> marks all «rows/columns» reached by rows i
...
!*** indices
!--------------------------------------
    IF (symm) CALL manymanyobtainsymm(na, ia, ja, va, ias, jas, vas)
...
!-----------------------------
    IF (refresh) THEN
      ...
      IF (symm) THEN
        CALL pardiso(pt, maxfct, mnum, mtype, phase, na, vas, ias, jas, &
...
!> adds repeated, etc..
  SUBROUTINE manymanycleansupnumerical(na, ia, ja, a)
    IMPLICIT NONE
    INTEGER :: na
    INTEGER:: ja(*), ia(na+1), indu(na), iwk(na+1)
    REAL(8), DIMENSION(*) :: a
    INTEGER :: i, j, k, ko, ipos, kfirst, klast
    REAL(8) :: tmp
!*** ensures the matrix is square
    IF (na .NE. manymanymaximumcolumn(na, ia, ja)) THEN
      STOP "in manymanycleansupnumerical, matrix must be square"
    END IF
    DO i = 1, na
      indu(i) = 0
      iwk(i) = ia(i)
    ...
    DO i = 1, na
      ...
      END DO
    END DO
  END SUBROUTINE manymanycleansupnumerical
!> example
!> list in
!> 5, 3, 1, 2, 4, ?
!> list out
...
!> obtains a subrelation
  SUBROUTINE manymanysubmatrixpart1(ia, ja, i1, i2, j1, j2, ib, jb)
! Input arguments are integers ia, ja, i1, i2, j1, j2 and integer arrays ib and jb
    IMPLICIT REAL(8) (a-h, o-z)
    INTEGER, DIMENSION(*) :: ia, ja
    INTEGER, DIMENSION(:), ALLOCATABLE :: ib, jb
    INTEGER :: nrb, ncb, i, ii, k1, k2, l, j
! Compute the dimensions of the submatrix
    nrb = i2-i1+1
! Allocate memory for ib with size nrb+1
    CALL allocsafe(nrb+1, ib)
    ncb = j2-j1+1
    l = 0
! Loop over all rows of the submatrix
    DO i = 1, nrb
      ...
      k2 = ia(ii+1)-1
      ib(i) = l+1
! Loop over all non-zero elements in the row
      DO k = k1, k2
        j = ja(k)
! Check if the column index is within the desired submatrix
        IF (j >= j1 .AND. j <= j2) THEN
          l = l+1
        END IF
      END DO
    END DO
! Allocate memory for jb with size l
    CALL allocsafe(l, jb)
    l = 0
! Loop over all rows of the submatrix for the second time
    DO i = 1, nrb
      ii = i1+i-1
      k1 = ia(ii)
      k2 = ia(ii+1)-1
      ib(i) = l+1
! Loop over all non-zero elements in the row for the second time
      DO k = k1, k2
        j = ja(k)
! Check if the column index is within the desired submatrix
        IF (j >= j1 .AND. j <= j2) THEN
    ...
    END DO
    ib(nrb+1) = l+1
  END SUBROUTINE manymanysubmatrixpart1
!> with a subrelation, obtains a submatrix
  SUBROUTINE manymanysubmatrixpart2(ia, ja, va, i1, i2, j1, j2, ib, vb)
    IMPLICIT REAL(8) (a-h, o-z)
    INTEGER, DIMENSION(*)::ia, ja
    REAL(8), DIMENSION(*)::va
    ...
    DO i = 1, nrb
      ...
      END DO
    END DO
  END SUBROUTINE manymanysubmatrixpart2
!> obtains a subrelation and submatrix
  SUBROUTINE manymanysubmatrixnumeric(ia, ja, va, i1, i2, j1, j2, ib, jb, vb)
    IMPLICIT REAL(8) (a-h, o-z)
    INTEGER, DIMENSION(*)::ia, ja
    REAL(8), DIMENSION(*)::va
    INTEGER, DIMENSION(:), ALLOCATABLE::ib, jb
    REAL(8), DIMENSION(:), ALLOCATABLE::vb
    CALL manymanysubmatrixpart1(ia, ja, i1, i2, j1, j2, ib, jb)
    CALL manymanysubmatrixpart2(ia, ja, va, i1, i2, j1, j2, ib, vb)
  END SUBROUTINE manymanysubmatrixnumeric
...
!> hyperbolic secant
  ...
  END FUNCTION sech
!> eigenvalues
!> of a voigt form (stress convention!)
  SUBROUTINE stressstraineigenvalues(com, vlp, vcp)
    IMPLICIT REAL(8) (a-h, o-z)
    ...
    DO j = 1, 3
      DO i = 1, 3
        CALL voigtfrommatrix(i, j, ic, 3)
        ten(i, j) = com(ic)
      END DO
    END DO
    CALL matrixeigensystem(ten, vlp, vcp, 3, ier)
  END SUBROUTINE stressstraineigenvalues
...
!> z=ax+by
  SUBROUTINE vectaxpby(n, z, a, x, b, y)
    ...
    IF (a .EQ. 0.0d00 .AND. b .EQ. 0.0d00) THEN
      CALL vectsetconst(n, z)
    ELSEIF (a .EQ. 0.0d00 .AND. b .NE. 0.0d00) THEN
      CALL vectscale(n, z, b, y)
    ELSEIF (a .NE. 0.0d00 .AND. b .EQ. 0.0d00) THEN
      CALL vectscale(n, z, a, x)
    ELSE
      IF (a .EQ. 1.0d00) THEN
        CALL vectaxpy(n, z, b, y, x)
      ELSEIF (b .EQ. 1.0d00) THEN
        CALL vectaxpy(n, z, a, x, y)
      ELSE
        DO i = 1, n
          rtemp = x(i)
          stemp = y(i)
