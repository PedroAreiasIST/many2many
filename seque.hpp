...
template<typename V, size_t S, auto P>
void setsize(seque<V, S, P> &container, size_t newSize, double growthFactor = 1.5) {
...
template<typename V, size_t S, auto P>
void erase(seque<V, S, P> &container, seque<size_t, S, P> const &indexContainer) {
...
template<typename V, size_t S, auto P>
void eraseinplace(seque<V, S, P> &container, size_t startIndex, size_t numToErase = 1) {
...
template<typename V, size_t S, auto P>
void addinplace(seque<V, S, P> &destContainer, size_t insertIndex, seque<V, S, P> const &sourceContainer) {
...
template<typename V, size_t S, auto P>
size_t append(seque<V, S, P> &destContainer, seque<V, S, P> const &sourceContainer) {
    const size_t oldSize = destContainer.size;
    setsize(destContainer, destContainer.size + sourceContainer.size);
    std::copy(destContainer.parallel, sourceContainer.actual, sourceContainer.actual + sourceContainer.size,
...
size_t append(seque<V, S, P> &destContainer, seque<V, S, P> &&sourceContainer) {
    const size_t oldSize = destContainer.size;
    setsize(destContainer, destContainer.size + sourceContainer.size);
    std::move(sourceContainer.actual, sourceContainer.actual + sourceContainer.size,
              destContainer.actual + oldSize);
    return oldSize - 1;
}
...
template<typename V, size_t S, auto P>
seque<size_t, S, P> indicesfromcondition(seque<V, S, P> const &container, auto predicate) {
...
seque<size_t, S, P> indicesfromvalue(seque<V, S, P> const &sortedcontainer, V value) {
    auto rangeResult = std::equal_range(sortedcontainer.actual,
...
auto getmapped(seque<V, S, P> const &container, auto func) {
    ...
    std::transform(container.parallel, container.actual,
                   container.actual + container.size, mappedContainer.actual, func);
    return mappedContainer;
}

template<typename V, size_t S, auto P>
auto getmapped(seque<V, S, P> const &containerA, seque<V, S, P> const &containerB, auto func) {
    ...
    std::transform(containerA.parallel, containerA.actual, containerA.actual + minSize,
                   containerB.actual, mappedContainer.actual, func);
    return mappedContainer;
}
...
template<typename V, size_t S, auto P>
auto dotproduct(seque<V, S, P> const &containerA, seque<V, S, P> const &containerB,
                auto init, auto sumFunc, auto prodFunc) {
    return std::inner_product(containerA.actual, containerA.actual + containerA.size,
...
seque<V, S, P> getintersection(seque<V, S, P> const &sortedcontainerA,
    ...
    auto endIt = std::set_intersection(sortedcontainerA.actual,
                                       sortedcontainerA.actual + sortedcontainerA.size,
                                       sortedcontainerB.actual,
                                       sortedcontainerB.actual + sortedcontainerB.size,
...
seque<V, S, P> getunion(seque<V, S, P> const &sortedcontainerA,
                        seque<V, S, P> const &sortedcontainerB) {
    seque<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
    auto endIt = std::set_union(sortedcontainerA.actual,
                                sortedcontainerA.actual + sortedcontainerA.size,
                                sortedcontainerB.actual,
                                sortedcontainerB.actual + sortedcontainerB.size,
                                resultContainer.actual);
...
seque<V, S, P> getdifference(seque<V, S, P> const &sortedcontainerA,
                             seque<V, S, P> const &sortedcontainerB) {
    seque<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
    auto endIt = std::set_difference(sortedcontainerA.actual,
                                     sortedcontainerA.actual + sortedcontainerA.size,
                                     sortedcontainerB.actual,
                                     sortedcontainerB.actual + sortedcontainerB.size,
                                     resultContainer.actual);
...
seque<V, S, P> getsymmetricdifference(seque<V, S, P> const &sortedcontainerA,
                                      seque<V, S, P> const &sortedcontainerB) {
    seque<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
    auto endIt = std::set_symmetric_difference(sortedcontainerA.actual,
                                               sortedcontainerA.actual + sortedcontainerA.size,
                                               sortedcontainerB.actual,
                                               sortedcontainerB.actual + sortedcontainerB.size,
                                               resultContainer.actual);
...
bool getincludessubset(seque<V, S, P> const &sortedsuperset,
    ...
    return std::includes(sortedsuperset.actual,
                         sortedsuperset.actual + sortedsuperset.size,
                         sortedsubset.actual,
