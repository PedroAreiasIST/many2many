...
struct sek
{
    ...
    sek(sek const &other) : sek() { copy_from(other); }
    sek(sek &&other) noexcept : sek() { move_from(std::move(other)); }
    ~sek() { setsize(*this, 0); }
...
template<typename V, size_t S, auto P>
void setordered(sek<V, S, P> &container)
{
    std::stable_sort(container.actual, container.actual + container.size);
}
template<typename V, size_t S, auto P>
void setordered(sek<V, S, P> &container, auto compareFunc)
{
...
template<typename V, size_t S, auto P>
auto getmapped(sek<V, S, P> const &container, auto func)
{
...
template<typename V, size_t S, auto P>
auto getmapped(sek<V, S, P> const &containerA, sek<V, S, P> const &containerB, auto func)
{
