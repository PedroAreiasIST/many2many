...
#pragma omp parallel for
    ...
    explicit sequence(size_t newSize) : sequence() { setsize(*this, newSize); }
    sequence(size_t newSize, V value) : sequence(newSize)
    {
#ifdef _OPENMP
#pragma omp parallel for
#endif
    ...
    sequence(std::initializer_list<V> initList) : sequence(initList.size())
    {
        std::move(initList.begin(), initList.end(), actual);
    }
    sequence(sequence const &other) : sequence() { copy_from(other); }
    sequence(sequence &&other) noexcept : sequence() { move_from(std::move(other)); }
    ~sequence() { setsize(*this, 0); }
...
#pragma omp parallel for
    ...
    sequence &operator=(std::initializer_list<V> initList)
    {
    ...
    sequence operator()(sequence<size_t, S, P> const &indexContainer) const
    {
        sequence result(indexContainer.size);
...
public:
    void _explodeinvalidindex(size_t index) const
    {
        if (index >= size)
        {
            throw std::out_of_range("sek: index " + std::to_string(index) + " out of range [0, " +
                                    std::to_string(size) + ").");
        }
    }
...
#pragma omp parallel for
    ...
    void _switchfromheapstostack()
    {
        ...
        if (heapdata)
        {
            std::move(heapdata, heapdata + moveCount, stackdata);
            arraydestroy(heapdata);
            heapdata = nullptr;
        }
#ifdef _OPENMP
...
inline V *begin(sequence<V, S, P> &container)
{
    return container.begin();
}
template<typename V, size_t S, auto P>
inline V *end(sequence<V, S, P> &container)
{
    return container.end();
}
template<typename V, size_t S, auto P>
void setsize(sequence<V, S, P> &container, size_t newSize, double growthFactor = 1.5)
{
    ...
    if (container.actual == container.stackdata)
    {
        ...
        {
            container._switchfromstacktoheap(static_cast<size_t>(std::llround(growthFactor * newSize)));
        }
    } else
    {
        if (newSize <= container.stacksize)
        ...
        {
            container._modifysizeofheap(static_cast<size_t>(std::llround(growthFactor * newSize)));
        }
    }
    container.size = newSize;
}
template<typename V, size_t S, auto P>
void save(auto archiver, sequence<V, S, P> const &container)
{
...
template<typename V, size_t S, auto P>
void load(auto archiver, sequence<V, S, P> &container)
{
    size_t loadedSize = 0;
    archiver(loadedSize);
...
template<typename V, size_t S, auto P>
void swap(sequence<V, S, P> &containerA, sequence<V, S, P> &containerB)
{
...
template<typename V, size_t S, auto P>
bool isindexvalid(sequence<V, S, P> const &container, size_t index)
{
    return (index < container.size);
}
template<typename V, size_t S, auto P>
void erase(sequence<V, S, P> &container)
{
    setsize(container, 0);
}
template<typename V, size_t S, auto P>
void erase(sequence<V, S, P> &container, size_t eraseIndex)
{
...
template<typename V, size_t S, auto P>
void erase(sequence<V, S, P> &container, sequence<size_t, S, P> const &indexContainer)
{
...
template<typename V, size_t S, auto P>
void eraselast(sequence<V, S, P> &container)
{
    if (container.size > 0)
    {
        setsize(container, container.size - 1);
...
template<typename V, size_t S, auto P>
void eraseinplace(sequence<V, S, P> &container, size_t startIndex, size_t numToErase = 1)
{
...
template<typename V, size_t S, auto P>
void add(sequence<V, S, P> &container, size_t insertIndex, V value)
{
...
template<typename V, size_t S, auto P>
void addinplace(sequence<V, S, P> &container, size_t insertIndex, V value)
{
...
template<typename V, size_t S, auto P>
void addinplace(sequence<V, S, P> &destContainer, size_t insertIndex, sequence<V, S, P> const &sourceContainer)
{
...
template<typename V, size_t S, auto P>
void append(sequence<V, S, P> &container, V value)
{
    setsize(container, container.size + 1);
    container[container.size - 1] = value;
}
template<typename V, size_t S, auto P>
void append(sequence<V, S, P> &destContainer, sequence<V, S, P> const &sourceContainer)
{
    const size_t oldSize = destContainer.size;
    setsize(destContainer, destContainer.size + sourceContainer.size);
    std::copy(destContainer.parallel, sourceContainer.actual, sourceContainer.actual + sourceContainer.size,
...
template<typename V, size_t S, auto P>
void append(sequence<V, S, P> &destContainer, sequence<V, S, P> &&sourceContainer)
{
    const size_t oldSize = destContainer.size;
    setsize(destContainer, destContainer.size + sourceContainer.size);
...
template<typename V, size_t S, auto P>
size_t getsize(sequence<V, S, P> const &container)
{
    return container.size;
}
template<typename V, size_t S, auto P>
bool operator<(sequence<V, S, P> const &lhs, sequence<V, S, P> const &rhs)
{
...
bool operator==(sequence<V, S, P> const &lhs, sequence<V, S, P> const &rhs)
{
    if (lhs.size != rhs.size)
        return false;
    return std::equal(lhs.parallel, lhs.actual, lhs.actual + lhs.size, rhs.actual);
}
template<typename V, size_t S, auto P>
bool operator!=(sequence<V, S, P> const &lhs, sequence<V, S, P> const &rhs)
{
...
template<typename V, size_t S, auto P>
std::ostream &operator<<(std::ostream &os, sequence<V, S, P> const &container)
{
...
template<typename V, size_t S, auto P>
std::istream &operator>>(std::istream &is, sequence<V, S, P> &container)
{
...
template<typename V, size_t S, auto P>
bool isfullysatisfied(sequence<V, S, P> const &container, auto predicate)
{
    return std::all_of(container.actual, container.actual + container.size, predicate);
}
template<typename V, size_t S, auto P>
sequence<size_t, S, P> indicesfromcondition(sequence<V, S, P> const &container, auto predicate)
{
    sequence<size_t, S, P> indices;
...
template<typename V, size_t S, auto P>
sequence<size_t, S, P> indicesfromvalue(sequence<V, S, P> const &container, V value,
                                        sequence<size_t, S, P> const &sortedIndices)
{
    size_t containerSize = container.size;
    if (containerSize == 0)
    {
        return sequence<size_t, S, P>{};
    }
    size_t left = 0;
    size_t right = containerSize - 1;
    sequence<size_t, S, P> indices(containerSize);
...
template<typename V, size_t S, auto P>
sequence<size_t, S, P> indicesfromvalue(sequence<V, S, P> const &sortedcontainer, V value)
{
    ...
    sequence<size_t, S, P> indices(rangeSize);
#ifdef _OPENMP
#pragma omp parallel for
#endif
...
template<typename V, size_t S, auto P>
sequence<size_t, S, P> getorder(sequence<V, S, P> const &container)
{
    sequence<size_t, S, P> sortedIndices(container.size);
...
template<typename V, size_t S, auto P>
sequence<V, S, P> getasample(sequence<V, S, P> const &container, size_t sampleSize)
{
    if (sampleSize > container.size)
    {
        sampleSize = container.size;
    }
    sequence<V, S, P> sampleContainer(sampleSize);
...
template<typename V, size_t S, auto P>
auto getmapped(sequence<V, S, P> const &containerA, sequence<V, S, P> const &containerB, auto func)
{
    const size_t minSize = (std::min) (containerA.size, containerB.size);
    using ReturnType = decltype(func(std::declval<V>(), std::declval<V>()));
    sequence<ReturnType, S, P> mappedContainer(minSize);
...
template<typename V, size_t S, auto P>
auto reduce(sequence<V, S, P> const &container, auto init, auto binaryFunc)
{
    return std::reduce(container.parallel, container.actual, container.actual + container.size, init, binaryFunc);
}
template<typename V, size_t S, auto P>
auto dotproduct(sequence<V, S, P> const &containerA, sequence<V, S, P> const &containerB, auto init, auto sumFunc,
                auto prodFunc)
{
...
template<typename V, size_t S, auto P>
void setshuffled(sequence<V, S, P> &container)
{
    std::random_device rd;
    std::mt19937 rng(rd());
...
template<typename V, size_t S, auto P>
void setreversed(sequence<V, S, P> &container)
{
    std::reverse(container.actual, container.actual + container.size);
}
template<typename V, size_t S, auto P>
void setrotatedaroundindex(sequence<V, S, P> &container, size_t rotationIndex)
{
...
template<typename V, size_t S, auto P>
bool setpermuteclockwise(sequence<V, S, P> &container)
{
    return std::next_permutation(container.actual, container.actual + container.size);
}
template<typename V, size_t S, auto P>
bool setpermutecounterclockwise(sequence<V, S, P> &container)
{
    return std::prev_permutation(container.actual, container.actual + container.size);
}
template<typename V, size_t S, auto P>
void setunique(sequence<V, S, P> &sortedcontainer)
{
    auto newEnd = std::unique(sortedcontainer.actual, sortedcontainer.actual + sortedcontainer.size);
    setsize(sortedcontainer, static_cast<size_t>(newEnd - sortedcontainer.actual));
}
template<typename V, size_t S, auto P>
void setorderedandunique(sequence<V, S, P> &container)
{
    setordered(container);
    setunique(container);
}
template<typename V, size_t S, auto P>
void indicesfromorder(sequence<V, S, P> const &sourceContainer, sequence<size_t, S, P> const &sortedIndices,
                      sequence<size_t, S, P> &mappingIndices, sequence<size_t, S, P> &firstSortedIndices)
{
    ...
    for (size_t index = 1; index < totalSize; ++index)
    {
        ...
        bool sameVal = (sourceContainer[sortedIndices[index]] == sourceContainer[sortedIndices[index - 1]]);
        firstSortedIndices[currentSortedIndex] = sameVal ? firstOccurrence : currentSortedIndex;
        if (!sameVal)
        {
...
template<typename V, size_t S, auto P>
sequence<V, S, P> getintersection(sequence<V, S, P> const &sortedcontainerA, sequence<V, S, P> const &sortedcontainerB)
{
    sequence<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
...
template<typename V, size_t S, auto P>
sequence<V, S, P> getunion(sequence<V, S, P> const &sortedcontainerA, sequence<V, S, P> const &sortedcontainerB)
{
    sequence<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
...
template<typename V, size_t S, auto P>
sequence<V, S, P> getdifference(sequence<V, S, P> const &sortedcontainerA, sequence<V, S, P> const &sortedcontainerB)
{
    sequence<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
...
template<typename V, size_t S, auto P>
sequence<V, S, P> getsymmetricdifference(sequence<V, S, P> const &sortedcontainerA,
                                         sequence<V, S, P> const &sortedcontainerB)
{
    sequence<V, S, P> resultContainer(sortedcontainerA.size + sortedcontainerB.size);
...
template<typename V, size_t S, auto P>
bool getincludessubset(sequence<V, S, P> const &sortedsuperset, sequence<V, S, P> const &sortedsubset)
{
...
template<typename V, size_t S, auto P>
size_t getindextoinsert(sequence<V, S, P> const &sortedcontainer, V value)
{
...
template<typename V, size_t S, auto P>
size_t addretainingorder(sequence<V, S, P> &sortedcontainer, V value)
{
