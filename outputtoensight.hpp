...
void ensightfromdb(mm2m const &m)
{
    ...
    for (int i = 0; i < getsize(aaa); ++i)
    {
        ...
        switch (etype)
        {
            ...
            case tet:
                ...
                for (int j = 0; j < 4; ++j)
                {
                    elno[elni[i] + j] = m(tet, node).nfrome[e][j];
                }
                break;
            case wedge:
                nelpr[i] = 7;
                ...
                for (int j = 0; j < 6; ++j)
                {
                    elno[elni[i] + j] = m(wedge, node).nfrome[e][j];
                }
                break;
            case hex:
                nelpr[i] = 6;
                ...
                for (int j = 0; j < 8; ++j)
                {
                    elno[elni[i] + j] = m(hex, node).nfrome[e][j];
                }
                break;
            default:
                break;
    ...
    ensightoutput(
        ...
        coordinates, nele, nomes, nelpr, ncnos, elni, elno); // Node coordinates: one std::array<double,3> per node
}
